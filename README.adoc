...
the tests need to `cf_scdf.properties`, which is loaded by the test process.
...
link:cf-scdf-setup.sh[cf-at.sh] is the common script that all SCDF Acceptance tests running in CI will run.
...
Load `cf_scdf.properties`, perform any additional setup, and run a maven command.
